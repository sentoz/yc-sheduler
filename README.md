...
- `http://localhost:9090/` — информация о сборке приложения (JSON с версией, коммитом, временем сборки)
...
- Запускается с интервалом, заданным в `validation_interval` (по умолчанию 10 минут)
- Определяет ожидаемое состояние ресурса на основе последних времен выполнения
  действий `start` и `stop` из расписания
- Если последнее действие `stop` было позже последнего `start`, ресурс должен быть
